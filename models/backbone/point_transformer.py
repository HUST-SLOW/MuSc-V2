...
from utils.pointnet2_utils import KNN_cpu
from models.modules._IPG import iter_group


def fps(data, number, fps_idx=None):
    '''
        data B N 3
        number int
    '''
...
class Group(nn.Module):
    def __init__(self, num_group, group_size):
        ...
        self.knn = KNN_cpu(k=int(group_size*2), transpose_mode=True)
    ...
    def forward(self, xyz):
        ...
        device = xyz.device
        _, idx_all = self.knn(xyz.cpu(), xyz.cpu())
        idx_all = idx_all.to(device)
        idx, curvatures, neighborhood = iter_group(xyz, center_idx, idx_all, self.group_size)
        assert idx.size(1) == self.num_group
        assert idx.size(2) == self.group_size
        ori_idx = idx
        return neighborhood, center, ori_idx, center_idx, curvatures
...
class PointTransformer(nn.Module):
    ...
    def forward(self, pts):
        ...
        neighborhood, center, ori_idx, center_idx, curvatures = self.group_divider(pts)
        ...
        # layer divide
        x = [f.permute(0,2,1) for f in feature_list]
        center = center.permute(0,2,1)
        return x, center, ori_idx, center_idx, curvatures
