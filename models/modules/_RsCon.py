...
def WMO(W, score, k_list=[1, 7]):
    # Window Mask Operation
    S_list = []
    for k in k_list:
...
def RsCon(scores_old, cls_tokens=None, k_list=[0]):
    ...
    scores_new = WMO(similarity_matrix.clone().float(), score=torch.tensor(scores).clone().float(), k_list=k_list)
